...
void test_set() {
  ...
  { // some complicated case...
    ...
    EXPECT_EQ(192, std::accumulate(M1.data(),M1.data()+M1.size(),int(0),
		[](auto const& a, auto& v) {return a+int(std::abs(v));}));
    EXPECT_EQ(96, std::count_if(M1.data(),M1.data()+M1.size(),
		[](auto& v) {return std::abs(v)>1.0e-6;}));
  }
}
